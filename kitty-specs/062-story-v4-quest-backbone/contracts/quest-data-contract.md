...
Legende: **T→N** setzt `advanceAct: N` · **D** Doppel-Tonspur im Abschluss · **N** neue Quest · Flag in `{…}`.
...
| id | npcId | prerequisites | objective | Marker/Flags |
|---|---|---|---|---|
| `magistrat_verification` | aldric | harren_daughter_investigation | fetch verification_seal ×1 | Wahl {verification_sealed / verification_refused} |
...
| `faction_campaign` | aldric | harren_daughter_investigation | fetch proclamation ×3 | **N**, optional; Wahl (factionStanding) |
| `council_collusion_reveal` | harren | magistrat_verification, klerus_purification, garde_patrol_expansion, widerstand_proof | observe collusion_reveal_seen ×1 | **T→2** |
...
| `garde_night_escort` | stadtwache | — | observe escort_route ×1 | **N D** |
...
| `who_you_were` | branka | branka_doubt | fetch memory_shard ×3, minDepth 5 | setzt {self_remembered} |
